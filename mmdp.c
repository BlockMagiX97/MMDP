...
void* _mmdp_chache_ser_sconfig=NULL;
uint32_t _mmdp_chache_ser_sconfig_size=0;
...
int deserialize_capability(const void* buf, uint32_t size, struct mmdp_capability* out) {
	...
	for (i=0;i<out->custom_struct_num;i++) {
		...
		/* checked size beforehand */
		out->custom_structs[i].net_name = strndup((char*)ptr, len);
		ptr += len+1;
		size -= len+1;
...
int init_mmdp_lib(uint32_t c_struct_num, struct mmdp_custom_struct* c_structs, int is_server) {
	...
	/* it is a server */
	_mmdp_chache_ser_sconfig = serialize_capability(&_mmdp_chache_ser_sconfig_size);
	if (_mmdp_chache_ser_sconfig == NULL) {
...
int init_connection_config_server(int fd, struct mmdp_serverside_config* conf_dest, void* write_context, void* read_context) {
	...
	if (mmdp_write(fd, _mmdp_chache_ser_sconfig, _mmdp_chache_ser_sconfig_size, write_context) != 0) {
